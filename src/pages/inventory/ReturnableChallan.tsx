  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-3xl font-bold tracking-tight text-slate-800">Returnable Challan Register</h2>
          <p className="text-slate-500 font-medium">Log, monitor, and print official Returnable Delivery Challans for subcontracting services and job work activities.</p>
        </div>
          ...
          <DialogContent className="sm:max-w-[700px] max-h-[90vh] overflow-y-auto">
            <form onSubmit={handleCreateChallanSubmit}>
              ...
              <div className="space-y-4 py-4 text-xs">
                ...
                <div className="grid grid-cols-3 gap-3">
                  <div className="space-y-1">
                    <label className="font-semibold text-slate-600">Challan Number</label>
                    <Input placeholder="e.g. RD000003810" value={newChallanNo} onChange={e => setNewChallanNo(e.target.value)} required />
                  </div>
                  <div className="space-y-1">
                    <label className="font-semibold text-slate-600">Expected Return Date</label>
                    <Input type="date" value={newExpectedDate} onChange={e => setNewExpectedDate(e.target.value)} required />
                  </div>
                  <div className="space-y-1">
                    <label className="font-semibold text-slate-600">Job Work Order Ref</label>
                ...
                <div className="grid grid-cols-2 gap-4 border-t pt-2">
                  <div className="space-y-2">
                    <h4 className="font-bold text-slate-700">Buyer (M/S) Details</h4>
                    <div className="space-y-1">
                  ...
                  <div className="space-y-2">
                    <h4 className="font-bold text-slate-700">Consignee (M/S) Details</h4>
                    <div className="space-y-1">
                    ...
                    <div className="text-[10px] text-slate-400">Leave blank if Consignee address is identical to Buyer address.</div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-3 border-t pt-2">
                  <div className="space-y-1 col-span-3">
                    <label className="font-semibold text-slate-600">Dispatch Purpose Description</label>
                ...
                <div className="space-y-3 pt-2 border-t">
                  <div className="flex justify-between items-center">
                    <h4 className="font-bold text-slate-700">Challan Items List</h4>
                    <Button type="button" variant="outline" size="sm" onClick={handleAddNewItemField}>
                      + Add Item Row
                    </Button>
                  </div>
                  {newItems.map((item, index) => (
                    <div key={index} className="flex gap-2 items-start bg-slate-50 p-2 rounded border border-dashed">
                      <div className="w-28 space-y-1">
                        <label className="text-[10px] text-slate-400">Item Code</label>
                      ...
                      <div className="flex-1 space-y-1">
                        <label className="text-[10px] text-slate-400">Item Details / Specs</label>
                        <textarea 
                          placeholder="specs, motors, model, serial no..." 
                          value={item.description} 
                          className="w-full text-xs p-2 bg-white border border-slate-200 rounded focus:outline-none focus:ring-1 focus:ring-primary/20 h-16"
                          onChange={e => handleItemFieldChange(index, 'description', e.target.value)}
                          required 
                        />
                      </div>
                      <div className="w-20 space-y-1">
                        <label className="text-[10px] text-slate-400">HSN/SAC</label>
                      ...
                      <div className="w-16 space-y-1">
                        <label className="text-[10px] text-slate-400">UOM</label>
              ...
              <DialogFooter>
                ...
                <Button type="submit" className="bg-blue-600 hover:bg-blue-700 text-white">Generate Returnable Challan</Button>
              </DialogFooter>
            </form>
          </DialogContent>
        </Dialog>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <Card className="hover:shadow-md transition-shadow">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-sm font-semibold text-slate-500">Total Issued</CardTitle>
            <ArrowLeftRight className="h-4 w-4 text-blue-500" />
          </CardHeader>
          <CardContent>
            <div className="text-3xl font-bold text-slate-800">{totalRDCs}</div>
            <p className="text-xs text-slate-400">Outward returnable gate passes</p>
          </CardContent>
        </Card>

        <Card className="hover:shadow-md transition-shadow">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-sm font-semibold text-slate-500">Outstanding Challans</CardTitle>
            <RefreshCcw className="h-4 w-4 text-amber-500" />
          </CardHeader>
          <CardContent>
            <div className="text-3xl font-bold text-amber-600">{pendingRDCs}</div>
            <p className="text-xs text-slate-400">Waiting material return logs</p>
          </CardContent>
        </Card>

        <Card className="hover:shadow-md transition-shadow">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-sm font-semibold text-slate-500">Overdue Reminders</CardTitle>
        ...
        <Card className="hover:shadow-md transition-shadow">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-sm font-semibold text-slate-500">Returned Clearance</CardTitle>
          ...
          <CardContent>
            ...
            <p className="text-xs text-slate-400">100% material closed</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader className="border-b pb-4">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <CardTitle className="text-lg font-semibold text-slate-800">Returnable Challan Register</CardTitle>
                ...
                return (
                  <TableRow key={challan.id} className="hover:bg-slate-50/50">
                    <TableCell className="font-semibold text-blue-600 text-xs">
                      {challan.id}
                      <span className="block text-[10px] text-slate-400 font-normal mt-0.5">{challan.jobWorkNo}</span>
                    </TableCell>
                    <TableCell>{format(new Date(challan.dateIssued), 'dd MMM yyyy')}</TableCell>
                    <TableCell className="font-medium text-slate-700">{challan.buyerName}</TableCell>
                    <TableCell className="text-slate-500 text-xs max-w-[200px] truncate">{challan.purpose}</TableCell>
                    <TableCell>
                      <span className="font-semibold text-slate-700">{totalPendingItemsCount.toFixed(1)}</span>
                      <span className="text-slate-400 text-xs font-normal"> units due</span>
                    ...
                    <TableCell className="text-xs">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className={isItemOverdue ? 'text-red-500 font-semibold' : 'text-slate-600'}>
                    ...
                    <TableCell className="text-right space-x-1">
                      <Button variant="ghost" size="sm" className="h-8 text-blue-600 hover:text-blue-800" onClick={() => handleOpenPrintPreview(challan)}>
          ...
          {selectedChallan && (
            <form onSubmit={handleReturnSubmit}>
              <DialogHeader>
                ...
                <DialogDescription>
                  Enter returning quantities for items sent to <span className="font-semibold text-slate-800">{selectedChallan.buyerName}</span>.
                </DialogDescription>
              </DialogHeader>
              <div className="space-y-4 py-4 text-xs">
                <div className="bg-slate-50 p-3 rounded-md border text-slate-600 space-y-1">
                ...
                <div className="space-y-3">
                  <h4 className="text-sm font-semibold text-slate-700">Enter Quantities Received back:</h4>
                    ...
                    return (
                      <div key={item.id} className="flex justify-between items-center border-b pb-2">
                        <div className="flex-1 pr-4">
                          ...
                          <p className="text-[10px] text-slate-500 whitespace-pre-line leading-normal truncate max-w-[250px]">
                        ...
                        <div className="w-24">
                          <label className="block text-[10px] text-slate-400 text-right mb-0.5">Recv Qty</label>
                          <Input 
                            type="number" 
                            step="0.001"
                            min="0"
              ...
              <DialogFooter>
                ...
                <Button type="submit" className="bg-blue-600 hover:bg-blue-700 text-white">Save Return Entry</Button>
              </DialogFooter>
            </form>
          )}
        </DialogContent>
        ...
        <DialogContent className="sm:max-w-[760px] max-h-[95vh] overflow-y-auto p-0 border-0 bg-transparent print-page-wrapper">
          {printChallan && (
            <div className="bg-white p-6 rounded-md shadow-xl border relative print-page-wrapper">
              ...
              <div className="border border-slate-800 p-5 bg-white text-slate-900 text-[11px] font-sans leading-relaxed min-h-[900px] flex flex-col justify-between print-doc-container">
                
                {/* Header Container */}
                <div>
                  <div className="flex border-b border-slate-800 pb-3">
                    {/* Left logo emblem */}
                    <div className="w-1/4 flex flex-col items-center justify-center border-r border-slate-800 pr-3">
                    ...
                    <div className="w-1/2 px-4 text-center flex flex-col justify-center">
                      ...
                      <p className="text-[9px] text-slate-500 font-semibold">(Formerly Sumesh Petroleum)</p>
                      <p className="text-[9px] font-bold text-slate-700 mt-1 uppercase tracking-wider">ISO 9001 : 2015 Certified Company</p>
                      <p className="text-[9px] text-slate-600 font-medium">CIN NO : U29309GJ2018PTC102237</p>
                      <p className="text-[9px] font-bold text-[#143157] mt-0.5">GST NO : 24ABACS2821F1ZT</p>
                    </div>

                    {/* Right Address & TABU logo */}
                    <div className="w-1/4 pl-3 border-l border-slate-800 flex flex-col justify-center text-left text-[9px] text-slate-600 space-y-0.5">
                      ...
                      <p className="font-semibold text-slate-800">226 - 227, G.I.D.C. Estate, Makarpura,</p>
                  ...
                  <div className="text-center font-bold tracking-widest uppercase border-b border-slate-800 py-1.5 bg-slate-50 text-xs text-slate-800">
                    RETURNABLE CHALLAN
                  </div>

                  {/* Buyer & Consignee details grid */}
                  <div className="grid grid-cols-2 border-b border-slate-800 text-[10px] divide-x divide-slate-800">
                    {/* Buyer Side */}
                    <div className="p-3 space-y-1">
                      <p className="font-bold text-slate-500 uppercase tracking-wider text-[9px]">Buyer's Name & Address</p>
                      <p className="font-bold text-[#143157] text-[11px]">M/S.  {printChallan.buyerName}</p>
                      <p className="text-slate-600 leading-tight whitespace-pre-wrap">{printChallan.buyerAddress}</p>
                      <p className="pt-2 font-medium"><span className="font-semibold">Ph. No. :</span> {printChallan.buyerPhone}</p>
                      <p className="font-bold text-slate-800"><span className="font-semibold">GSTIN :</span> {printChallan.buyerGstin}</p>
                      <p className="text-slate-400">Kind. Attn. :</p>
                    </div>

                    {/* Consignee & Challan Numbers Side */}
                    <div className="flex flex-col">
                      {/* Top Part: Challan meta */}
                      <div className="p-2 border-b border-slate-800 grid grid-cols-2 text-[10px] bg-slate-50/50">
                        <div><span className="font-bold text-slate-600">Challan No.</span> &nbsp;&nbsp;&nbsp;: <span className="font-bold text-slate-900">{printChallan.id}</span></div>
                        <div><span className="font-bold text-slate-600">Challan Date</span> : <span className="font-bold text-slate-900">{format(new Date(printChallan.dateIssued), 'dd/MM/yyyy')}</span></div>
                      </div>
                      {/* Bottom Part: Consignee name/address */}
                      <div className="p-3 flex-1 space-y-1">
                        <p className="font-bold text-slate-500 uppercase tracking-wider text-[9px]">Consignee's Name & Address</p>
                        <p className="font-bold text-[#143157] text-[11px]">M/S.  {printChallan.consigneeName}</p>
                        <p className="text-slate-600 leading-tight whitespace-pre-wrap">{printChallan.consigneeAddress}</p>
                      </div>
                    </div>
                  </div>

                  {/* Items Table - Rendered with full black borders */}
                  <table className="w-full text-left border-collapse border-b border-slate-800">
                    <thead>
                      <tr className="bg-slate-100 text-[10px] font-bold border-b border-slate-800">
                        <th className="border-r border-slate-800 py-2 px-2 text-center w-10">No.</th>
                        <th className="border-r border-slate-800 py-2 px-2 w-28">Item Code</th>
                        <th className="border-r border-slate-800 py-2 px-2">Item Details</th>
                        <th className="border-r border-slate-800 py-2 px-2 text-center w-24">HSN/SAC</th>
                        <th className="border-r border-slate-800 py-2 px-2 text-center w-16">UOM</th>
                        <th className="py-2 px-2 text-right w-20">Quantity</th>
                      </tr>
                    </thead>
                    <tbody>
                      {printChallan.items.map((item, index) => (
                        <tr key={item.id} className="text-[10px] align-top border-b border-slate-300">
                          <td className="border-r border-slate-800 py-2 px-2 text-center">{index + 1}</td>
                          <td className="border-r border-slate-800 py-2 px-2 font-mono font-semibold text-slate-700">{item.itemCode}</td>
                          <td className="border-r border-slate-800 py-2 px-2">
                            <div className="font-bold text-slate-800 leading-tight whitespace-pre-wrap font-sans">
                              {item.description}
                            </div>
                          </td>
                          <td className="border-r border-slate-800 py-2 px-2 text-center text-slate-500 font-semibold">{item.hsnSac}</td>
                          <td className="border-r border-slate-800 py-2 px-2 text-center text-slate-600">{item.uom}</td>
                          <td className="py-2 px-2 text-right font-bold text-slate-800">{item.qtyDispatched.toFixed(3)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ...
                <div className="mt-4 space-y-3">
                  {/* Remarks block */}
                  <div className="text-[9px] text-slate-700 space-y-1 border border-slate-800 p-2.5 rounded-sm bg-slate-50/50 leading-relaxed">
                    ...
                    <div className="grid grid-cols-2 pt-1.5 border-t border-slate-300 mt-1">
                      <div><span className="font-bold text-slate-800">Company's Operator Name:-</span> {printChallan.operatorName || '-'}</div>
                      <div className="text-right"><span className="font-bold text-slate-800">Driver's Detail's:-</span> {printChallan.driverDetails || '-'}</div>
                    </div>
                  </div>

                  {/* GSTIN / PAN & Signatory */}
                  <div className="grid grid-cols-2 text-[10px] border border-slate-800 divide-x divide-slate-800">
                    <div className="p-3 space-y-1.5 self-center">
                      <div><span className="font-bold text-slate-500">GSTIN</span> &nbsp;&nbsp;&nbsp;: <span className="font-bold text-slate-850">24ABACS2821F1ZT</span></div>
                      <div><span className="font-bold text-slate-500">PAN</span> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;: <span className="font-bold text-slate-850">ABACS2821F</span></div>
                    </div>
                    <div className="p-3 text-right flex flex-col justify-between h-20 bg-slate-50/30">
                      <p className="font-bold text-slate-750">For, Sumesh Petroleum Pvt. Ltd.</p>
                      <p className="text-[9px] font-bold text-slate-500 tracking-wider">Authorized Signatory</p>
                    </div>
                  </div>

                  {/* Transporter Details & Sign-offs */}
                  <div className="grid grid-cols-3 text-[9px] text-slate-600 gap-4 pt-2">
                    <div className="space-y-1 font-medium">
                      <div><span className="font-semibold">Transporter:</span> {printChallan.transporter || '-'}</div>
                      <div><span className="font-semibold">Vehicle No.:</span> <span className="font-bold text-slate-800">{printChallan.vehicleNo || '-'}</span></div>
                    ...
                    <div className="text-center flex flex-col justify-end h-16">
                      ...
                      <p className="text-[8px] text-slate-500">( {printChallan.preparedBy || 'chirag'} )</p>
                    </div>
                    <div className="text-center flex flex-col justify-end h-16">
                      <div className="border-b border-dashed border-slate-400 w-3/4 mx-auto mb-1"></div>
                      <p className="font-bold text-slate-700">Received By</p>
                      <p className="text-[8px] text-slate-500">( Signature & Stamp )</p>
                    </div>
                  </div>

                  {/* Page index footer */}
                  <div className="text-center text-[8px] text-slate-400 pt-2 border-t border-slate-250 flex justify-between uppercase font-semibold mt-3">
